  ...
  implicit def configReader(implicit spec: Spec): Reads[FullConfig] = {
    ...
    Json.reads[FullConfig].flatMap { case fullCfg =>
      val ps = fullCfg.tools.collectFirst { case tool if tool.name == spec.name =>
        if (tool.patterns.isEmpty) {
          JsError("no patterns selected")
        } else {
          val patterns = tool.patterns.filter(pattern => spec.patterns.exists(_.patternId == pattern.patternId))

          // only return my tool we don't need the others
          fullCfg.copy(tools = Set(tool.copy(patterns = patterns)))
          JsSuccess(fullCfg)
        }
